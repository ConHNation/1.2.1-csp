...
def get_names(file_name):
  leaderboard_file = open('leaderboard.txt', "r")  # be sure you have created this
  index = 0
...
def update_leaderboard(file_name, leader_names, leader_scores,  player_name, player_score):
  ...
  for index in range(len(leader_names)):
    ...
    print(type(leader_names[index]))
    ###Debug End
    player_score = int(player_score)
    cleader_score = int(leader_scores[index])
    ###Debug
    print(type(player_score))
    print(type(cleader_score))
    ###Debug End
    # TODO 9: check if this is the position to insert new score at
    if (player_score <= cleader_score):
      break
    else:
  ...
  leaderboard_file = open('leaderboard.txt', "w")  # this mode opens the file and erases its contents for a fresh start
 
  # TODO 13 loop through all the leaderboard elements and write them to the the file
  for index in leader_names:
